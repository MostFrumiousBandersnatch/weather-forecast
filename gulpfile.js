...
function compileLess() {
    return gulp.src(SRC_LESS_ALL)
        .pipe(require("gulp-less")({
            paths: [path.join(SRC_LESS_BASE, "includes")]
        }))
...
// Compile everything
gulp.task("compile", ["copy-bower", "compile:html", "compile:less", "compile:javascript"]);


// Dist everything
gulp.task("dist", ["copy-bower", "dist:html", "dist:less", "dist:javascript"]);
...
gulp.task("create-config", function () {
    var source = require('vinyl-source-stream'),
        stream = source('conf.js'),
        content = [
            "angular.module('weather_config', []).value(",
            "'WEATHER_CONFIG',",
            JSON.stringify(require('./conf.json')),
            ");"
        ].join('');

    stream.end(content);
    stream.pipe(gulp.dest(SRC_JAVASCRIPT_BASE));
});


// Updates the Bower dependencies based on the bower.json file
gulp.task("update", ["create-config"], function (next) {
